...
export class PostForm extends React.Component {
    ...
    Create = () => {
        ...
        fetch('http://' + SERVER_URL + '/examtype/', {
            method: 'POST',
...
export class PutForm extends React.Component {
    ...
    Update = (e) => {
        ...
        console.log(JSON.stringify(data))
        fetch('http://' + SERVER_URL + '/examtype/' + this.state.id + '/', {
            method: 'PUT',
            body: JSON.stringify(data),
            headers: {
                "Content-type": "application/json",
    ...
    Delete(e) {
        fetch('http://' + SERVER_URL + '/examtype/' + e.target.id, {
            method: 'DELETE',
            headers: {
                "Content-type": "application/json",
                'Accept': 'application/json',
        ...
        return (
            <div>
                <div className="form-group row">
                    <label htmlFor="examtype" className="col-4 col-form-label">Exam Type</label>
                    <div className="col-8">
                        <input id="examtype" name="examtype" key='examtype' value={this.state.put_data.examtype} onChange={this.handleChange} placeholder="Exam Type" type="text" required="required" className="form-control" />
                    </div>
                </div>
                <div className="form-group row">
                    <div className="offset-4 col-8">
