        ...
        return (
            <div>
                <div className="form-group row">
                    <label htmlFor="role" className="col-4 col-form-label">Add category</label>
                    <div className="col-8">
                        <input id="category" name="category" key='category' onChange={this.handleChange} placeholder="category" type="text" required="required" className="form-control" />
                    </div>
                </div>
                <div className="form-group row">
...
export class PutForm extends React.Component {
    ...
    Update = (e) => {
        e.preventDefault()
        fetch('http://' + SERVER_URL + '/api/category/' + this.props.select._id + '/', {
            method: 'PUT',
            credentials: 'include',
            ...
            .then(response => response.status)
            .then(async (data) => await (data == 202) ? window.location.reload() : alert('You can not update a sample'))
    }
    Delete(e) {
        fetch('http://' + SERVER_URL + '/api/category/' + this.props.select._id, {
            method: 'DELETE',
            ...
            .then(response => response.status)
            .then(async (data) => await (data == 204) ? window.location.reload() : alert('Not Successful'))
    }
    handleChange(e) {
        ...
        return (
            <div>
                <div className="form-group row">
                    <label htmlFor="role" className="col-4 col-form-label">Add category</label>
                    <div className="col-8">
                        <input id="category" name="category" key='category' placeholder={this.props.select.category} onChange={this.handleChange} type="text" required="required" className="form-control" />
                    </div>
                </div>
                <div className="form-group row">
                    <div className="offset-4 col-8">
