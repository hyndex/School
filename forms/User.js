...
export class PutForm extends React.Component {
    ...
    Update = (e) => {
        ...
        fetch('http://' + SERVER_URL + '/api/user/' + this.props.select._id + '/', {
            ...
            .then(response => response.status)
            .then(async (data) => await (data < 300) ? window.location.reload() : alert('You can not update'))
    }
    Delete(e) {
        fetch('http://' + SERVER_URL + '/api/user/' + this.props.select._id, {
            method: 'DELETE',
        ...
        return (
            <div>
                <div className="form-group row">
                    ...
                    <div className="col-8">
                        <input id="email" name="email" key='email' onChange={this.handleChange} defaultValue={this.props.select.email} type="text" required="required" className="form-control" />
                    </div>
                </div>
                <div className="form-group row">
                    <div className="offset-4 col-8">
